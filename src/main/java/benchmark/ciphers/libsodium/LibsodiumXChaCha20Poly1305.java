...
public class LibsodiumXChaCha20Poly1305 {

    private static final Linker LINKER = Linker.nativeLinker();
    private static final SymbolLookup LIBCRYPTO = LibsodiumLoader.loadLibcrypto();
    public static final MethodHandle crypto_aead_xchacha20poly1305_ietf_encrypt;
    public static final MethodHandle crypto_aead_xchacha20poly1305_ietf_decrypt;
    public static final MethodHandle sodium_init;
    private static final int CCPO_KEY_SIZE = 32;
    ...
    static {
         ...
         */
            ...
            crypto_aead_xchacha20poly1305_ietf_encrypt = LINKER
                    .downcallHandle(LIBCRYPTO.find("crypto_aead_xchacha20poly1305_ietf_encrypt").orElseThrow(), FunctionDescriptor.of(
         ...
         */
            ...
            crypto_aead_xchacha20poly1305_ietf_decrypt = LINKER
                    .downcallHandle(LIBCRYPTO.find("crypto_aead_xchacha20poly1305_ietf_decrypt").orElseThrow(), FunctionDescriptor.of(
    ...
    public static byte[] encryptOneShot(byte[] key, byte[] iv, byte[] input, int inputLen) {
        if (key == null || key.length != CCPO_KEY_SIZE) {
            throw new IllegalArgumentException("Invalid key length: expected " + CCPO_KEY_SIZE + " bytes");
        }
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            if (rc != 0) {
                throw new LibsodiumException("crypto_aead_xchacha20poly1305_ietf_encrypt failed");
            }
    ...
    public static byte[] decryptOneShot(byte[] key, byte[] iv, byte[] input, int inputLen) {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            if (rc != 0) {
                throw new LibsodiumException("crypto_aead_xchacha20poly1305_ietf_decrypt failed");
            }
    ...
    private LibsodiumXChaCha20Poly1305() {
    }
